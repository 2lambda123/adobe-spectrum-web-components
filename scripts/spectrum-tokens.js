...
const tokensRoot = path.join(__dirname, '..', 'node_modules', '@spectrum-css',
                             'tokens', 'dist', 'css', '**', '*.css');
 ...
 */
const tokenPackages = [
  'accordion',     'actionbar',      'actionbutton',
  'actiongroup',   'alertdialog',    'avatar',
  'badge',         'button',         'buttongroup',
  'checkbox',      'closebutton',    'colorarea',
  'colorhandle',   'colorloupe',     'colorwheel',
  'divider',       'dropzone',       'fieldgroup',
  'fieldlabel',    'helptext',       'illustratedmessage',
  'infieldbutton', 'link',           'menu',
  'picker',        'pickerbutton',   'popover',
  'progressbar',   'progresscircle', 'radio',
  'search',        'sidenav',        'slider',
  'splitview',     'statuslight',    'stepper',
  'swatch',        'swatchgroup',    'switch',
  'table',         'tabs',           'tag',
  'textfield',     'thumbnail',      'toast',
  'tooltip',       'tray',           'underlay',
];

const packagePaths = tokenPackages.map((packageName) => {
  return path.join(__dirname, '..', 'node_modules', '@spectrum-css',
                   packageName, 'dist', 'themes');
});
...
const processTokens = (srcPath) => {
  ...
  fs.writeFileSync(
      path.join(__dirname, '..', 'tools', 'styles', 'tokens', fileName), css);
};
...
const processPackages = async (srcPath, index) => {
  ...
  if (fs.existsSync(expressPath)) {
    ...
    fs.appendFileSync(path.join(__dirname, '..', 'tools', 'styles', 'tokens',
                                'express', 'global-vars.css'),
                      express);
  }
  ...
  if (fs.existsSync(spectrumPath)) {
    ...
    fs.appendFileSync(path.join(__dirname, '..', 'tools', 'styles', 'tokens',
                                'spectrum', 'global-vars.css'),
                      spectrum);
  }

  const varsPaths =
      path.join(__dirname, '..', 'tools', 'styles', '**', '*.css');
  const varsRegExp = new RegExp(`\\s*--spectrum-${packageName}[^;}]*;*`, 'g');
  const aliasRegExp =
      new RegExp(`\\s*--spectrum-alias-${packageName}[^;}]*;*`, 'g');
