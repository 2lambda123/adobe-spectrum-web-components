...
const processIcon = (srcPath, fd, scaleWidth, scaleHeight) => {
  ...
  // regex will extract width, height and svg content into $1, $2 and $3
  // respectively
  const regex =
      new RegExp(/<svg.*viewBox="(.*)">[\s|\n]*(.*?)[\s|\n]*<\/svg>/i);
  ...
  fs.writeSync(fd, `<symbol id="spectrum-icon-${iconName}" viewBox="${
                       viewBox}">${svgContent}</symbol>`);
};

// where is spectrum-css?
// TODO: use resolve package to find node_modules
const spectrumIconsPath = path.resolve(path.join(
    __dirname, '..', 'node_modules', '@spectrum-css', 'ui-icons', 'dist'));
...
Object.keys(scales).forEach((scaleKey) => {
  ...
  const outputPath = path.join(__dirname, '..', 'packages', 'icons', 'src',
